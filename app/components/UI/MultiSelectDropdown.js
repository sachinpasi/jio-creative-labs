...
const MultiSelectDropdown = ({ label, placeholder, options = [] }) => {
  ...
  return (
    <div className="relative inline-block w-full" ref={dropdownRef}>
      ...
      <div className="mt-2 relative">
        ...
        {isOptionOpen && (
          <div className="origin-top-right absolute z-30 right-0 mt-2 w-full  shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="py-1">
              {options.map((option, idx) => (
                <button
                  key={idx}
                  className={`w-full text-left px-4 py-2 text-[22px] flex items-center  ${
                    isSelected(idx)
